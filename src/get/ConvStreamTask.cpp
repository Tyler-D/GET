...
ConvStreamTask<Dtype>::~ConvStreamTask()
{
...
ConvStreamTask<Dtype> :: PreCompute()
{
	...
	while(input_height > width_filter)
	{
		...
		DataBlob<Dtype> filter((void *)this->datas_[1].host_data(),1,1,this->datas_[1].height(),this->datas_[1].width());
		...
		this->inner_datas_.push_back(filter);
    ...
    cl_int datasize_f = filter_h_*filter_w_*sizeof(Dtype);
    ...
    temp_buffer = clCreateBuffer(this->device_->DeviceContext(), CL_MEM_READ_ONLY, datasize_f, NULL, &status);
...
ConvStreamTask<Dtype>::Compute()
{
    ...
    if (typeid(Dtype) == typeid(int))
        program_source = cl_readSource("/home/zy/GET/src/get/conv_task_int.cl");
    if (typeid(Dtype) == typeid(long))
        program_source = cl_readSource("/home/zy/GET/src/get/conv_task_long.cl");
    if (typeid(Dtype) == typeid(float))
        program_source = cl_readSource("/home/zy/GET/src/get/conv_task_float.cl");
    if (typeid(Dtype) == typeid(double))
        program_source = cl_readSource("/home/zy/GET/src/get/conv_task_double.cl");
    ...
    int padding_pixels = (int)(filter_w_ / 2) * 2;
    ...
    {
         kernel[i] = clCreateKernel(program, "convMatrix", &status);
        ...
        status = clSetKernelArg(kernel[i], 1, sizeof(cl_mem), &(this->device_buffers_[2]));
        cl_pStatus(status, "clSetKernelArg_1");
        status = clSetKernelArg(kernel[i], 2, sizeof(cl_mem), &(this->device_buffers_[1]));
        cl_pStatus(status, "clSetKernelArg_2");
        status = clSetKernelArg(kernel[i], 3, sizeof(cl_int), &out_height_each);
        cl_pStatus(status, "clSetKernelArg_3");
        status = clSetKernelArg(kernel[i], 4, sizeof(cl_int), &out_width_each);
        cl_pStatus(status, "clSetKernelArg_4");
        status = clSetKernelArg(kernel[i], 5, sizeof(cl_int), &in_height_each);
        cl_pStatus(status, "clSetKernelArg_5");
        status = clSetKernelArg(kernel[i], 6, sizeof(cl_int), &in_width_each);
        cl_pStatus(status, "clSetKernelArg_6");
        status = clSetKernelArg(kernel[i], 7, sizeof(cl_uint), &filter_w_);
        cl_pStatus(status, "clSetKernelArg_7");
        status = clSetKernelArg(kernel[i], 8, local_mem, NULL);
        cl_pStatus(status, "clSetKernelArg_8");
        status = clSetKernelArg(kernel[i], 9, sizeof(cl_uint), &local_height);
        cl_pStatus(status, "clSetKernelArg_9");
        status = clSetKernelArg(kernel[i], 10, sizeof(cl_uint), &local_width);
        cl_pStatus(status, "clSetKernelArg_10");
      
    }

    int in_height_left = this->inner_datas_[2*i].height();
    int in_width = this->inner_datas_[2*i].width();
    ...
    status = clSetKernelArg(kernel[i], 2, sizeof(cl_mem), &(this->device_buffers_[1]));
    cl_pStatus(status, "clSetKernelArg_2");
    status = clSetKernelArg(kernel[i], 3, sizeof(cl_int), &out_height);
    cl_pStatus(status, "clSetKernelArg_3");
    status = clSetKernelArg(kernel[i], 4, sizeof(cl_int), &out_width);
    cl_pStatus(status, "clSetKernelArg_4");
    status = clSetKernelArg(kernel[i], 5, sizeof(cl_int), &in_height_left);
    cl_pStatus(status, "clSetKernelArg_5");
    status = clSetKernelArg(kernel[i], 6, sizeof(cl_int), &in_width);
    cl_pStatus(status, "clSetKernelArg_6");
    status = clSetKernelArg(kernel[i], 7, sizeof(cl_uint), &filter_w_);
    cl_pStatus(status, "clSetKernelArg_7");
    status = clSetKernelArg(kernel[i], 8, local_mem, NULL);
    ...
    for(int i = 0; i < blocks_num ; i+=2)
    {
        ...
        status = clEnqueueWriteBuffer(this->device_->DeviceCmdQueA(),this->device_buffers_[0], CL_FALSE, 0, this->inner_datas_[2*i].height()*data_w_*sizeof(Dtype), (Dtype*)(this->inner_datas_[2*i].host_data()), 0, NULL, NULL );
        cl_pStatus(status, "clEnqueueWriteBuffer");
        status = clEnqueueWriteBuffer(this->device_->DeviceCmdQueA(),this->device_buffers_[1], CL_FALSE, 0, this->inner_datas_[2*i+1].height()*filter_w_*sizeof(Dtype), (Dtype*)(this->inner_datas_[2*i+1].host_data()), 0, NULL, NULL );
        cl_pStatus(status, "clEnqueueWriteBuffer");
        ...
        {
            status = clEnqueueWriteBuffer(this->device_->DeviceCmdQueB(),this->device_buffers_[0], CL_FALSE, 0, this->inner_datas_[2*(i+1)].height()*data_w_*sizeof(Dtype), (Dtype*)(this->inner_datas_[2*(i+1)].host_data()), 0, NULL, NULL );
            cl_pStatus(status, "clEnqueueWriteBuffer");
            status = clEnqueueWriteBuffer(this->device_->DeviceCmdQueB(),this->device_buffers_[1], CL_FALSE, 0, this->inner_datas_[2*(i+1)+1].height()*filter_w_*sizeof(Dtype), (Dtype*)(this->inner_datas_[2*(i+1)+1].host_data()), 0, NULL, NULL );
            cl_pStatus(status, "clEnqueueWriteBuffer");
            status = clEnqueueNDRangeKernel(this->device_->DeviceCmdQueB(), kernel[i+1] , 2, NULL , global_worksize, local_worksize, 0, NULL, NULL );
            cl_pStatus(status, "clEnqueueNDRangeKernel");
        }

        status = clEnqueueReadBuffer(this->device_->DeviceCmdQueA(),this->device_buffers_[2], CL_FALSE, 0, this->inner_results_[i].height()*output_w_*sizeof(Dtype), (Dtype*)(this->inner_results_[i].host_data()), 0, NULL, NULL );
        ...
        {
            status = clEnqueueReadBuffer(this->device_->DeviceCmdQueB(),this->device_buffers_[2], CL_FALSE, 0, this->inner_results_[i+1].height()*output_w_*sizeof(Dtype), (Dtype*)(this->inner_results_[i+1].host_data()), 0, NULL, NULL );
        cl_pStatus(status, "clEnqueueReadBuffer");       
        }        
    }
...
template <typename Dtype>
void 
ConvStreamTask<Dtype>::PostCompute()
{
}


template class ConvStreamTask<int>;
template class ConvStreamTask<long>;
template class ConvStreamTask<float>;
template class ConvStreamTask<double>;
