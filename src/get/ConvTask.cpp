...
#define LOCAL_WORKGROUP_HEIGHT 4
#define LOCAL_WORKGROUP_WIDTH 4


template <typename Dtype>
void
ConvTask<Dtype>::PreCompute()
{
    //set device buffer
    cl_mem temp_buffer;
    cl_int status;
    ...
    temp_buffer = clCreateBuffer(this->device_->DeviceContext(), CL_MEM_READ_ONLY, filter_h_*filter_w_*sizeof(Dtype), NULL, &status);
    ...
    //copy data from host to device
     ...
     cout<<"filter address : " <<this->datas_[1].host_data() <<endl;
    ...
    status = clEnqueueWriteBuffer(this->device_->DeviceCmdQueA(),this->device_buffers_[1], CL_TRUE, 0, filter_h_*filter_w_*sizeof(Dtype), (Dtype*)(this->datas_[1].host_data()), 0, NULL, NULL );
    cl_pStatus(status, "clEnqueueWriteBuffer");
}

template <typename Dtype>
void
ConvTask<Dtype>::Compute()
{
   //read program source and create kernel
    ...
    kernel = clCreateKernel(program, "convMatrix", &status);
   ...
   //set kernel arg
    ...
    status = clSetKernelArg(kernel, 1, sizeof(cl_mem), &(this->device_buffers_[2]));
    cl_pStatus(status, "clSetKernelArg_1");
    status = clSetKernelArg(kernel, 2, sizeof(cl_mem), &(this->device_buffers_[1]));
    cl_pStatus(status, "clSetKernelArg_2");
    status = clSetKernelArg(kernel, 3, sizeof(cl_int), &output_h_);
    cl_pStatus(status, "clSetKernelArg_3");
    status = clSetKernelArg(kernel, 4, sizeof(cl_int), &output_w_);
    cl_pStatus(status, "clSetKernelArg_4");
    status = clSetKernelArg(kernel, 5, sizeof(cl_int), &data_h_);
    cl_pStatus(status, "clSetKernelArg_5");
    status = clSetKernelArg(kernel, 6, sizeof(cl_int), &data_w_);
    cl_pStatus(status, "clSetKernelArg_6");
    status = clSetKernelArg(kernel, 7, sizeof(cl_uint), &filter_w_);
    cl_pStatus(status, "clSetKernelArg_7");
    status = clSetKernelArg(kernel, 8, local_mem, NULL);
    cl_pStatus(status, "clSetKernelArg_8");
    status = clSetKernelArg(kernel, 9, sizeof(cl_uint), &local_height);
    cl_pStatus(status, "clSetKernelArg_9");
    status = clSetKernelArg(kernel, 10, sizeof(cl_uint), &local_width);
...
template <typename Dtype>
void
ConvTask<Dtype>::PostCompute()
{
    //copy memory from device to host
    cl_int status; 
    status = clEnqueueReadBuffer(this->device_->DeviceCmdQueA(),this->device_buffers_[2], CL_TRUE, 0, output_h_*output_w_*sizeof(Dtype), (Dtype*)(this->results_[0].host_data()), 0, NULL, NULL );
...
template class ConvTask<int>;
template class ConvTask<float>;
template class ConvTask<double>;
template class ConvTask<long>;
