...
MulStreamTask<Dtype>::~MulStreamTask()
{
...
template <typename Dtype>
void 
MulStreamTask<Dtype> :: PreCompute()
{
    ...
    //create inner blocks 
    unsigned int height_each = M_ / temp_num;
    ...
    for(int i = 0; i < temp_num - 1 ; i++)
    {
        ...
        inner.ReshapeLike(1, 1, height_each, K_);
        inner.CopyFromMemory(this->datas_[0].host_data()+offset*K_);
        this->inner_datas_.push_back(inner);
        inner.ReshapeLike(1, 1, K_, N_);
        inner.CopyFromMemory(this->datas_[1].host_data());
        this->inner_datas_.push_back(inner);
        
        inner.ReshapeLike(1, 1, height_each, N_);
        inner.CopyFromMemory(this->results_[0].host_data()+offset*N_);
    ...
    height_left = M_ - height_left;
    DataBlob<Dtype> inner;
    inner.ReshapeLike(1, 1, height_left, K_);
    inner.CopyFromMemory(this->datas_[0].host_data()+offset*K_);
    this->inner_datas_.push_back(inner);
    inner.ReshapeLike(1, 1, K_, N_);
    inner.CopyFromMemory(this->datas_[1].host_data());
    ...
    inner.ReshapeLike(1, 1, height_left, N_);
    inner.CopyFromMemory(this->results_[0].host_data()+offset*N_);
    this->inner_results_.push_back(inner);
    ...
    temp_buffer = clCreateBuffer(this->device_->DeviceContext(), CL_MEM_READ_ONLY, height_each*K_*sizeof(Dtype) , NULL, &status);
    ...
    this->device_buffers_.push_back(temp_buffer);
    temp_buffer = clCreateBuffer(this->device_->DeviceContext(), CL_MEM_WRITE_ONLY, height_each*N_*sizeof(Dtype), NULL, &status);
...
MulStreamTask<Dtype>::Compute()
{
    ...
    {
         kernel[i] = clCreateKernel(program, "mulMatrix", &status);
        ...
        status = clSetKernelArg(kernel[i], 2, sizeof(cl_mem), &(this->device_buffers_[2]));
        cl_pStatus(status, "clSetKernelArg_2");
        status = clSetKernelArg(kernel[i], 3, sizeof(cl_uint), &height_each);
        cl_pStatus(status, "clSetKernelArg_3");
        status = clSetKernelArg(kernel[i], 4, sizeof(cl_uint), &K_);
        cl_pStatus(status, "clSetKernelArg_4");
        status = clSetKernelArg(kernel[i], 5, sizeof(cl_uint), &N_);
        cl_pStatus(status, "clSetKernelArg_5");
        status = clSetKernelArg(kernel[i], 6, sizeof(cl_uint), &thread_division_row);
        cl_pStatus(status, "clSetKernelArg_6");
        status = clSetKernelArg(kernel[i], 7, sizeof(cl_uint), &thread_division_col);
    ...
    kernel[i] = clCreateKernel(program, "mulMatrix", &status);
    cl_pStatus(status, "clCreateKernel");
    status = clSetKernelArg(kernel[blocks_num-1], 0, sizeof(cl_mem), &(this->device_buffers_[0]));
    cl_pStatus(status, "clSetKernelArg_0");
    status = clSetKernelArg(kernel[blocks_num-1], 1, sizeof(cl_mem), &(this->device_buffers_[1]));
    cl_pStatus(status, "clSetKernelArg_1");
    status = clSetKernelArg(kernel[blocks_num-1], 2, sizeof(cl_mem), &(this->device_buffers_[2]));
    cl_pStatus(status, "clSetKernelArg_2");
    status = clSetKernelArg(kernel[blocks_num-1], 3, sizeof(cl_uint), &height_left);
    cl_pStatus(status, "clSetKernelArg_3");
    status = clSetKernelArg(kernel[blocks_num-1], 4, sizeof(cl_uint), &K_);
    cl_pStatus(status, "clSetKernelArg_4");
    status = clSetKernelArg(kernel[blocks_num-1], 5, sizeof(cl_uint), &N_);
    cl_pStatus(status, "clSetKernelArg_5");
    status = clSetKernelArg(kernel[blocks_num-1], 6, sizeof(cl_uint), &thread_division_row);
    cl_pStatus(status, "clSetKernelArg_6");
    status = clSetKernelArg(kernel[blocks_num-1], 7, sizeof(cl_uint), &thread_division_col);
    cl_pStatus(status, "clSetKernelArg_7");
    ...
    {
        status = clEnqueueWriteBuffer(this->device_->DeviceCmdQueA(),this->device_buffers_[0], CL_FALSE, 0, this->inner_datas_[2*i].height()*K_*sizeof(Dtype), (Dtype*)(this->inner_datas_[2*i].host_data()), 0, NULL, NULL );
        cl_pStatus(status, "clEnqueueWriteBuffer");
        status = clEnqueueWriteBuffer(this->device_->DeviceCmdQueA(),this->device_buffers_[1], CL_FALSE, 0, this->inner_datas_[2*i+1].height()*N_*sizeof(Dtype), (Dtype*)(this->inner_datas_[2*i+1].host_data()), 0, NULL, NULL );
        cl_pStatus(status, "clEnqueueWriteBuffer");
        ...
        {
            status = clEnqueueWriteBuffer(this->device_->DeviceCmdQueB(),this->device_buffers_[0], CL_FALSE, 0, this->inner_datas_[2*(i+1)].height()*K_*sizeof(Dtype), (Dtype*)(this->inner_datas_[2*(i+1)].host_data()), 0, NULL, NULL );
            cl_pStatus(status, "clEnqueueWriteBuffer");
            status = clEnqueueWriteBuffer(this->device_->DeviceCmdQueB(),this->device_buffers_[1], CL_FALSE, 0, this->inner_datas_[2*(i+1)+1].height()*N_*sizeof(Dtype), (Dtype*)(this->inner_datas_[2*(i+1)+1].host_data()), 0, NULL, NULL );
            cl_pStatus(status, "clEnqueueWriteBuffer");
            status = clEnqueueNDRangeKernel(this->device_->DeviceCmdQueB(), kernel[i+1] , 2, NULL , global_worksize, local_worksize, 0, NULL, NULL );
            cl_pStatus(status, "clEnqueueNDRangeKernel");
        }

        status = clEnqueueReadBuffer(this->device_->DeviceCmdQueA(),this->device_buffers_[2], CL_FALSE, 0, this->inner_results_[i].height()*N_*sizeof(Dtype), (Dtype*)(this->inner_results_[i].host_data()), 0, NULL, NULL );
        ...
        {
            status = clEnqueueReadBuffer(this->device_->DeviceCmdQueB(),this->device_buffers_[2], CL_FALSE, 0, this->inner_results_[i+1].height()*N_*sizeof(Dtype), (Dtype*)(this->inner_results_[i+1].host_data()), 0, NULL, NULL );
        cl_pStatus(status, "clEnqueueReadBuffer");       
        }        
    }
...
template <typename Dtype>
void 
MulStreamTask<Dtype>::PostCompute()
{
}


template class MulStreamTask<int>;
template class MulStreamTask<long>;
template class MulStreamTask<float>;
template class MulStreamTask<double>;
