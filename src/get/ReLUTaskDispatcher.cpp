/*************************************************************************
	> File Name: ReLUTaskDispatcher.cpp
	> Author: 
	> Mail: 
...
void 
ReLUTaskDispatcher<Dtype>::PreTaskDispatch()
{
    ...
    DataBlob<Dtype> blob;
    blob.ReshapeLike(1, 1, relu_param_.height(), relu_param_.width());
        //prepare input
...
template <typename Dtype>
void 
ReLUTaskDispatcher<Dtype>::TaskDispatch()
{
    //get device : mem_used
    long mem_used = 0;
    ...
    */ 
    if (device_enough)
    {
        ...
        for( i = 0; i < this->devices_needed_.size()-1;i++)
        {
            ...
            height_each = relu_param_.height() * weight[i];
            height_current += height_each;
            ReLUTask<Dtype>* temp_task = new ReLUTask<Dtype>(this->devices_needed_[i]);
            temp_task->SetParams(1, height_each, relu_param_.width());
            //create inner
            DataBlob<Dtype> inner;
            inner.ReshapeLike(1, 1, height_each, relu_param_.width());
            ...
            cout<<"create " << i << " relutask" <<endl;  
            offset += height_each*(relu_param_.width())*sizeof(Dtype);
        } // end for 

            height_each = relu_param_.height() - height_current;
            ReLUTask<Dtype>* temp_task = new ReLUTask<Dtype>(this->devices_needed_[i]);
            temp_task->SetParams(1, height_each, relu_param_.width());
            //create inner
            DataBlob<Dtype> inner;
    ...
    else
    {
        ...
        ReLUStreamTask<Dtype>* temp_adst = new ReLUStreamTask<Dtype>(stream_device);
        temp_adst->SetParams(1, relu_param_.height(), relu_param_.width());
...
template class ReLUTaskDispatcher<int>;
template class ReLUTaskDispatcher<long>;
template class ReLUTaskDispatcher<float>;
template class ReLUTaskDispatcher<double>;
