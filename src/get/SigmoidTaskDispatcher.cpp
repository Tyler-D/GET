/*************************************************************************
	> File Name: SigmoidTaskDispatcher.cpp
	> Author: 
	> Mail: 
...
void 
SigmoidTaskDispatcher<Dtype>::PreTaskDispatch()
{
    ...
    DataBlob<Dtype> blob;
    blob.ReshapeLike(1, 1, sig_param_.height(), sig_param_.width());
        //prepare input
...
template <typename Dtype>
void 
SigmoidTaskDispatcher<Dtype>::TaskDispatch()
{
    //get device : mem_used
    long mem_used = 0;
    ...
    */ 
    if (device_enough)
    {
        ...
        for( i = 0; i < this->devices_needed_.size()-1;i++)
        {
            ...
            height_each = sig_param_.height() * weight[i];
            height_current += height_each;
            SigmoidTask<Dtype>* temp_task = new SigmoidTask<Dtype>(this->devices_needed_[i]);
            temp_task->SetParams(1, height_each, sig_param_.width());
            //create inner
            DataBlob<Dtype> inner;
            inner.ReshapeLike(1, 1, height_each, sig_param_.width());
            ...
            offset += height_each*(sig_param_.width())*sizeof(Dtype);
        } // end for 

            height_each = sig_param_.height() - height_current;
            SigmoidTask<Dtype>* temp_task = new SigmoidTask<Dtype>(this->devices_needed_[i]);
            temp_task->SetParams(1, height_each, sig_param_.width());
            //create inner
            DataBlob<Dtype> inner;
    ...
    else
    {
        ...
        SigmoidStreamTask<Dtype>* temp_adst = new SigmoidStreamTask<Dtype>(stream_device);
...
template class SigmoidTaskDispatcher<int>;
template class SigmoidTaskDispatcher<long>;
template class SigmoidTaskDispatcher<float>;
template class SigmoidTaskDispatcher<double>;
