...
SubStreamTask<Dtype>::~SubStreamTask()
{
...
template <typename Dtype>
void 
SubStreamTask<Dtype> :: PreCompute()
{
    ...
    for(int i = 0; i < temp_num - 1 ; i++)
    {
        ...
        inner.CopyFromMemory(this->datas_[1].host_data()+offset);
        this->inner_datas_.push_back(inner);
...
template <typename Dtype>
void
SubStreamTask<Dtype>::Compute()
{
    cl_int status;
    
    if (typeid(Dtype) == typeid(int))
        char* program_source = cl_readSource("sub_task_int.cl");
    if (typeid(Dtype) == typeid(long))
        char* program_source = cl_readSource("sub_task_long.cl");
    if (typeid(Dtype) == typeid(float))
        char* program_source = cl_readSource("sub_task_float.cl");
    if (typeid(Dtype) == typeid(double))
        char* program_source = cl_readSource("sub_task_double.cl");
    ...
    {
        kernel[i] = clCreateKernel(program, "subMatrix", &status);
        cl_pStatus(status, "clCreateKernel");
        status = clSetKernelArg(kernel[i], 0, sizeof(cl_mem), &(this->device_buffers_[2]));
        cl_pStatus(status, "clSetKernelArg_0");
        status = clSetKernelArg(kernel[i], 1, sizeof(cl_int), &width_);
        cl_pStatus(status, "clSetKernelArg_1");
        status = clSetKernelArg(kernel[i], 2, sizeof(cl_int), &height_each);
        cl_pStatus(status, "clSetKernelArg_2");
        status = clSetKernelArg(kernel[i], 3, sizeof(cl_uint), &thread_division_row);
        cl_pStatus(status, "clSetKernelArg_3");
        status = clSetKernelArg(kernel[i], 4, sizeof(cl_uint), &thread_division_col);
        cl_pStatus(status, "clSetKernelArg_4");
        status = clSetKernelArg(kernel[i], 5, sizeof(cl_mem), &(this->device_buffers_[0]));
        cl_pStatus(status, "clSetKernelArg_5");
        status = clSetKernelArg(kernel[i], 6, sizeof(cl_mem), &(this->device_buffers_[1]));
    ...
    status = clSetKernelArg(kernel[i], 1, sizeof(cl_int), &width_);
    cl_pStatus(status, "clSetKernelArg_1");
    status = clSetKernelArg(kernel[i], 2, sizeof(cl_int), &height_left);
    cl_pStatus(status, "clSetKernelArg_2");
    status = clSetKernelArg(kernel[i], 3, sizeof(cl_uint), &thread_division_row);
    cl_pStatus(status, "clSetKernelArg_3");
    status = clSetKernelArg(kernel[i], 4, sizeof(cl_uint), &thread_division_col);
    ...
    {
        status = clEnqueueWriteBuffer(this->device_->DeviceCmdQueA(),this->device_buffers_[0], CL_FALSE, 0, this->inner_datas_[2*i].height()*width_*sizeof(Dtype), (Dtype*)(this->inner_datas_[2*i].host_data()), 0, NULL, NULL );
        cl_pStatus(status, "clEnqueueWriteBuffer");
        status = clEnqueueWriteBuffer(this->device_->DeviceCmdQueA(),this->device_buffers_[1], CL_FALSE, 0, this->inner_datas_[2*i+1].height()*width_*sizeof(Dtype), (Dtype*)(this->inner_datas_[2*i+1].host_data()), 0, NULL, NULL );
        cl_pStatus(status, "clEnqueueWriteBuffer");
        ...
        {
            status = clEnqueueWriteBuffer(this->device_->DeviceCmdQueB(),this->device_buffers_[0], CL_FALSE, 0, this->inner_datas_[2*(i+1)].height()*width_*sizeof(Dtype), (Dtype*)(this->inner_datas_[2*(i+1)].host_data()), 0, NULL, NULL );
            cl_pStatus(status, "clEnqueueWriteBuffer");
            status = clEnqueueWriteBuffer(this->device_->DeviceCmdQueB(),this->device_buffers_[1], CL_FALSE, 0, this->inner_datas_[2*(i+1)+1].height()*width_*sizeof(Dtype), (Dtype*)(this->inner_datas_[2*(i+1)+1].host_data()), 0, NULL, NULL );
            cl_pStatus(status, "clEnqueueWriteBuffer");
            status = clEnqueueNDRangeKernel(this->device_->DeviceCmdQueB(), kernel[i+1] , 2, NULL , global_worksize, local_worksize, 0, NULL, NULL );
            cl_pStatus(status, "clEnqueueNDRangeKernel");
        }

        status = clEnqueueReadBuffer(this->device_->DeviceCmdQueA(),this->device_buffers_[2], CL_FALSE, 0, this->inner_results_[i].height()*width_*sizeof(Dtype), (Dtype*)(this->inner_results_[i].host_data()), 0, NULL, NULL );
        ...
        {
            status = clEnqueueReadBuffer(this->device_->DeviceCmdQueB(),this->device_buffers_[2], CL_FALSE, 0, this->inner_results_[i+1].height()*width_*sizeof(Dtype), (Dtype*)(this->inner_results_[i+1].host_data()), 0, NULL, NULL );
        cl_pStatus(status, "clEnqueueReadBuffer");       
        }        
    }
...
template <typename Dtype>
void 
SubStreamTask<Dtype>::PostCompute()
{
}


template class SubStreamTask<int>;
template class SubStreamTask<long>;
template class SubStreamTask<float>;
template class SubStreamTask<double>;
