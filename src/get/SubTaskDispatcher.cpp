/*************************************************************************
	> File Name: SubTaskDispatcher.cpp
	> Author: 
	> Mail: 
...
void 
SubTaskDispatcher<Dtype>::PreTaskDispatch()
{
    ...
    DataBlob<Dtype> blob;
    blob.ReshapeLike(1, 1, sub_param_.height(), sub_param_.width());
        //prepare input
...
template <typename Dtype>
void 
SubTaskDispatcher<Dtype>::TaskDispatch()
{
    //get device : mem_used
    long mem_used = 0;
    ...
    */ 
    if (device_enough)
    {
        ...
        for( i = 0; i < this->devices_needed_.size()-1;i++)
        {
            ...
            height_each = sub_param_.height() * weight[i];
            height_current += height_each;
            SubTask<Dtype>* temp_task = new SubTask<Dtype>(this->devices_needed_[i]);
            temp_task->SetParams(1, height_each, sub_param_.width());
            //create inner
            DataBlob<Dtype> inner;
            inner.ReshapeLike(1, 1, height_each, sub_param_.width());
            ...
            cout<<"create " << i << " subtask" <<endl;  
            offset += height_each*(sub_param_.width())*sizeof(Dtype);
        } // end for 

            height_each = sub_param_.height() - height_current;
            SubTask<Dtype>* temp_task = new SubTask<Dtype>(this->devices_needed_[i]);
            temp_task->SetParams(1, height_each, sub_param_.width());
            //create inner
            DataBlob<Dtype> inner;
    ...
    else
    {
        ...
        SubStreamTask<Dtype>* temp_adst = new SubStreamTask<Dtype>(stream_device);
        temp_adst->SetParams(1, sub_param_.height(), sub_param_.width());
...
template class SubTaskDispatcher<int>;
template class SubTaskDispatcher<long>;
template class SubTaskDispatcher<float>;
template class SubTaskDispatcher<double>;
